...
/** Badge variant + matching text/icon token per confidence level. */
const LEVEL_CONFIG: Record<
  ConfidenceLevel,
  { variant: BadgeVariant; color: string }
> = {
  low: { variant: "error", color: "var(--text-error)" },
  medium: { variant: "warning", color: "var(--text-warning)" },
  high: { variant: "success", color: "var(--text-success)" },
};
...
export const ConfidenceBadge = forwardRef<HTMLSpanElement, ConfidenceBadgeProps>(
  (
    ...
    const { variant, color } = LEVEL_CONFIG[resolvedLevel];
    ...
    const badge = (
      <Badge
        ...
        icon={
          hideIcon ? undefined : (
            <Target color={color} weight="regular" aria-hidden="true" />
          )
        }
